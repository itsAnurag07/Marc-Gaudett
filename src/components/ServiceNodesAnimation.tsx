...
export default function ServiceNodesAnimation() {
  ...
  const nodes: Node[] = [
    ...
    },
    {
      ...
      y: 14,
      connections: [0, 1, 5],
      detail: "SaaS Growth Conversations",
    },
    {
      id: 3,
  ...
  return (
    <div className="w-full">
      ...
      <div className="hidden md:block relative w-full h-[400px] lg:h-[480px] bg-[#070b14] rounded-[24px] border border-white/5 overflow-hidden select-none">
      ...
      {/* Mobile view: Clean vertical grid fallback */}
      <div className="block md:hidden bg-[#070b14] rounded-[24px] border border-white/5 p-5 space-y-4 w-full select-none text-left">
        <div className="flex items-center gap-3 pb-3 border-b border-white/5">
          <div className="w-10 h-10 rounded-full bg-blue-600 border border-blue-400 flex items-center justify-center text-white shrink-0">
            <span className="material-symbols-outlined text-lg">hub</span>
          </div>
          <div>
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Marc Gaudett Hub</h3>
            <p className="text-[10px] text-gray-400">Revenue &amp; Growth Strategy</p>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-2.5">
          {nodes.filter(n => n.id !== 0).map((node) => (
            <div key={node.id} className="p-3 bg-white/5 border border-white/10 rounded-xl flex items-start gap-2.5">
              <span className="material-symbols-outlined text-[14px] text-blue-400 mt-0.5 shrink-0">check_circle</span>
              <div className="space-y-0.5">
                <span className="text-xs font-semibold text-gray-200 block">{node.label}</span>
                <span className="text-[10px] text-gray-400 block leading-normal">{node.detail}</span>
              </div>
            </div>
          ))}
        </div>
